...
typedef struct TkCanvas {
    ...
    int simple;			/* Non-zero means simple canvas drawing
 				 * routines. */
    ...
    TagSearchExpr *bindTagExprs;/* Linked list of tag expressions used in
				 * bindings. */
#endif
    void (*display_func)(ClientData) ; /* display function */
    void (*scrollbar_func)(struct TkCanvas *canvas);/* scrollbar function */
} TkCanvas;
 ...
 *----------------------------------------------------------------------
 *
 * Procedure prototypes and structures used for defining new fast canvas
 * drawing items
 *
 *----------------------------------------------------------------------
 */
typedef void (*Tk_scrollbar_func)(struct TkCanvas *canvas) ;
...
MODULE_SCOPE int 	TkCanvTranslatePath(TkCanvas *canvPtr,
			    ...
			    XPoint *outPtr);
/*
MODULE_SCOPE void *Tk_CanvasDispFunc( Tk_function func ) ;
MODULE_SCOPE void *Tk_CanvasScrollbarFunc( Tk_function func ) ;
MODULE_SCOPE void Tk_CanvasDispUpdate( void ) ;
*/
