...
static const Tk_OptionSpec commonOptSpec[] = {
    ...
    {TK_OPTION_INT, "-parasite", "parasite", "Parasite",
	"0", -1, -1, 
	TK_CONFIG_NULL_OK|TK_OPTION_DONT_SET_DEFAULT, 0, 0},
				 ...
				 * the new application. */
{
	...
	if ((arg[1] == 'c') && (length >= 3)
		...
		&& (strncmp(arg, "-parasite", length) == 0)) {
	    if( Tcl_GetIntFromObj(interp,objv[i+1],&para_val) != TCL_OK ){
	      Tcl_AppendResult( interp, "invalid parasite window id", NULL ) ;
	      return( TCL_ERROR ) ;
	    }
	    parasiteName = (Window) para_val ;
	}
    }
	...
	newWin = Tk_CreateWindowFromPath(interp, tkwin, Tcl_GetString(objv[1]), 
 		                         screenName);
 	Tk_CreateParasiteTopLevel(newWin,parasiteName) ;
    } else if (tkwin != NULL) {
	newWin = Tk_CreateWindowFromPath(interp, tkwin, Tcl_GetString(objv[1]),
		screenName);
    ...
    if ( (type == TYPE_TOPLEVEL) && !(parasiteName) ) {
