...
typedef struct TkPostscriptInfo {
    ...
    double xscale, yscale ;	/* Scale factor for conversion: each pixel
				 * maps into this many points. */
    double itools_scale ;	/* scale itools by this amount */
    ...
    int prolog;			/* Non-zero means output should contain the
				 ...
				 * ::tk::ps_preamable [sic]. */
    int nonuniform;		/* Non-zero means output should be nonuniform */
...
static const Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_BOOLEAN, "-nonuniform", NULL, NULL,
	"", Tk_Offset(TkPostscriptInfo, nonuniform), 0},
	...
	"", Tk_Offset(TkPostscriptInfo, itools_scale), 0},
...
#define STRING_LENGTH 400
    ...
    psInfo.width *= psInfo.itools_scale ;
    psInfo.height *= psInfo.itools_scale ;
	...
	if (GetPostscriptPoints(interp, psInfo.pageWidthString,
		&psInfo.xscale) != TCL_OK) {
	    goto cleanup;
	}
	psInfo.xscale /= psInfo.width;
    } else if (psInfo.pageHeightString != NULL) {
	if (GetPostscriptPoints(interp, psInfo.pageHeightString,
		&psInfo.xscale) != TCL_OK) {
	    goto cleanup;
	}
	psInfo.xscale /= psInfo.height;
    } else {
	psInfo.xscale = (72.0/25.4)*WidthMMOfScreen(Tk_Screen(tkwin));
	psInfo.xscale /= WidthOfScreen(Tk_Screen(tkwin));
    }
    psInfo.yscale = psInfo.xscale ; /* uniform case */
    if( psInfo.nonuniform ){
      if (psInfo.pageHeightString != NULL) {
	if (GetPostscriptPoints(interp, psInfo.pageHeightString,
		&psInfo.yscale) != TCL_OK) {
	    goto cleanup;
	}
	psInfo.yscale /= psInfo.height;
      }
    }
	...
	if (!psInfo.rotate) {
	    Tcl_AppendPrintfToObj(psObj,
		    "%%%%BoundingBox: %d %d %d %d\n",
		    (int) (psInfo.pageX + psInfo.xscale*deltaX),
		    (int) (psInfo.pageY + psInfo.yscale*deltaY),
		    (int) (psInfo.pageX + psInfo.xscale*(deltaX + psInfo.width)
			    + 1.0),
		    (int) (psInfo.pageY + psInfo.yscale*(deltaY + psInfo.height)
			    + 1.0));
	} else {
	    Tcl_AppendPrintfToObj(psObj,
		    "%%%%BoundingBox: %d %d %d %d\n",
		    (int) (psInfo.pageX - psInfo.xscale*(deltaY+psInfo.height)),
		    (int) (psInfo.pageY + psInfo.yscale*deltaX),
		    (int) (psInfo.pageX - psInfo.xscale*deltaY + 1.0),
		    (int) (psInfo.pageY + psInfo.yscale*(deltaX + psInfo.width)
			    + 1.0));
	}
	Tcl_AppendPrintfToObj(psObj,
		"%%%%Pages: 1\n"
 ...
 *--------------------------------------------------------------
 *
 * Tk_PostscriptChannel --
 *
 *	This procedure is called by individual canvas items when
 *	they want to know about the current about channel.
 *
 * Results:
 *	Returns the current channel for the canvas.
 *
 * Side effects:
 *	None.
 *
...
Tcl_Channel Tk_PostscriptChannel(Tk_Canvas canvas)
{
    TkCanvas *canvas_p ;
    TkPostscriptInfo *ps_info_p ;

    canvas_p = (TkCanvas *) canvas ;
    if( canvas_p->psInfo ){
      ps_info_p = (TkPostscriptInfo *) canvas_p->psInfo ;
      return( ps_info_p->chan ) ;
    }
    return(NULL) ;
}

    ...
    Tcl_Obj *psObj)		/* Where to append the postscript. */
{
     ...
     * matter here.
     * Well, it is much slower and now we do it when we absolutely must. WPS
     */
    error_flag = Tk_SizeOfBitmap(Tk_Display(tkwin), bitmap,&totalWidth,&totalHeight) ;
    if( error_flag == TCL_ERROR ){
    ...
    Pixmap bitmap)		/* Bitmap to use for stippling. */
{
    ...
    error_flag = Tk_SizeOfBitmap(Tk_Display(tkwin), bitmap, &width,&height) ;
    if( error_flag == TCL_ERROR ){
      XGetGeometry(Tk_Display(tkwin), bitmap, &dummyRoot,
