    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
	...
	EVENT_PLACE,	EVENT_ROOT,	EVENT_ROOTX,	EVENT_ROOTY, EVENT_SEND,
	EVENT_SENDEVENT,EVENT_SERIAL,	EVENT_STATE,	EVENT_SUBWINDOW,
 ...
 *      In addition, it records the event in the history.
 *
 * Results:
 ...
 *
 * Side effects:
 *      Command is put in the history list.
 *
 *----------------------------------------------------------------------
 */

int
TkCopyRecordAndGlobalEval(interp, script)
    ...
    char *script;			/* Script to evaluate. */
{
    ...
    code = Tcl_RecordAndEval(interp, Tcl_DStringValue(&buffer), TCL_EVAL_GLOBAL);
