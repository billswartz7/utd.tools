...
static Window parasite_windowS = 0 ;
static Display *parasite_displayS = NULL ;
static void (*global_window_creation_handlerG)(Display *disp,Window win) ;
static void (*global_window_deletion_handlerG)(Display *disp,Window win) ;
extern TkDisplay *TkpOpenParasiteDisplay(Display *display) ;
    ...
    int *screenPtr)		/* Where to store screen number. */
{
	...
	if (dispPtr == NULL) {
	     ...
	     */
  	    ...
  	    if( parasite_displayS ){
  	      dispPtr = TkpOpenParasiteDisplay(parasite_displayS);
  	    } else {
				 ...
				 * form "prog instance". */
{
    ...
    if( parasite_windowS ){
      int child ;			/* child counter */
      int new ;				/* return pointer */
      unsigned int num_children ;	/* number of children */
      XWindowAttributes wattr;		/* get window attributes of parasite */
      Window *childrenW ;		/* children windows */
      Window rootW ;			/* root window */
      Window parentW ;			/* root window */
      Tcl_HashEntry *hPtr ;		/* add window to winTable */

      winPtr->window = parasite_windowS ;

      /* -----------------------------------------------------------------
       * First unmap children of the parasite.
      ----------------------------------------------------------------- */
      if (XQueryTree(winPtr->display, winPtr->window, &rootW, &parentW,
		     &childrenW, &num_children) != 0) {
	for( child = 0 ; child < num_children ; child++ ){
	  XUnmapWindow(winPtr->display, childrenW[child]);
	}
	XFree((char *) childrenW) ;
      }

      /* -----------------------------------------------------------------
       * Now update the current window for use.
      ----------------------------------------------------------------- */

      winPtr->flags |= TK_MAPPED ;
      winPtr->flags |= TK_PARASITE ;
      XGetWindowAttributes( winPtr->display, winPtr->window, &wattr ) ;
      winPtr->changes.x = wattr.x ;
      winPtr->changes.y = wattr.y ;
      winPtr->changes.width = wattr.width ;
      winPtr->changes.height = wattr.height ;
      winPtr->changes.border_width = wattr.border_width ;
      winPtr->visual = wattr.visual ;
      winPtr->depth = wattr.depth ;
      winPtr->atts.colormap = wattr.colormap ;
      XSelectInput(winPtr->display,winPtr->window,
	(ALL_EVENTS_MASK)^StructureNotifyMask) ;
      TkWmParasite(winPtr) ;
    ...
    {
      CONST char *app_name ;
      app_name = Tk_SetAppName(tkwin, baseName) ;
      winPtr->nameUid = Tk_GetUid( app_name ) ;
      ckfree((char *)app_name) ;
    }
#endif /* TK_SEND */
 ...
 *--------------------------------------------------------------
 *
 * Tk_CreateParasiteTopLevel --
 *
 *	Swap out the window information of the top level window
 ...
 *--------------------------------------------------------------
 */
void Tk_CreateParasiteTopLevel(Tk_Window tkwin, Window window)
{
    int child ;				/* child counter */
    int new ;				/* return pointer */
    unsigned int num_children ;		/* number of children */
    ...
    Window parentW ;			/* root window */
    Tcl_HashEntry *hPtr ;		/* add window to winTable */
    TkWindow *winPtr;			/* current window info */
    ...
    Tk_Window tkwin)		/* Window to destroy. */
{
	...
	Tcl_DeleteHashEntry(Tcl_FindHashEntry(&dispPtr->winTable,
		(char *) winPtr->window));
	if( global_window_deletion_handlerG && !(winPtr->flags & TK_PARASITE) ){
 	  (*global_window_deletion_handlerG)(winPtr->display, winPtr->window) ;
 	}
	winPtr->window = None;
    }
    UnlinkWindow(winPtr);
    ...
    Tk_Window tkwin)		/* Token for window. */
{
    ...
    if( global_window_creation_handlerG ){
      (*global_window_creation_handlerG)(winPtr->display, winPtr->window) ;
    }
    ...
    Tcl_Interp *interp)		/* Interpreter to initialize. */
{
	...
	fprintf( stderr, "we are failing here\n") ;
	return TCL_ERROR;
    }
	...
	if (TCL_OK != Tcl_ListObjAppendList(interp, parseList, value) ||
	    ...
	    TCL_OK != Tcl_ParseArgsObjv(interp, table, &objc, objv, &rest,TCL_ARGV_NO_ABBREV)) {
...
void Tk_ParasiteDisplay(Display *display)
{
    parasite_displayS = display ;

} /* end Tk_ParasiteDisplay() */

void Tk_ParasiteWindow(Window parasite)
{
    parasite_windowS = parasite ;

} /* end Tk_ParasiteWindow() */


void Tk_ParasiteRestore(Tk_Window tkwin)
{
    Tcl_Interp *interp;		/* Interpreter to use for error reporting. */
    ...
    Tk_UpdateObjCmd(tkwin,interp,1,NULL) ;

} /* end Tk_ParasiteRestore() */

TkWindow **Tk_WindowChildren(TkWindow *tkwin, int *num_children)
{
    int count ;
    TkWindow *ptr ;
    TkWindow **array ;
    ...
    count = 0 ;
    array = NULL ;
    for( ptr = tkwin->childList; ptr ; ptr = ptr->nextPtr ){
      count++ ;
    }
    if( count ){
      array = (TkWindow **) ckalloc( count * sizeof(TkWindow *)) ;
      for( count = 0, ptr = tkwin->childList; ptr ; ptr = ptr->nextPtr ){
	array[count++] = ptr ;
      }
    }
    *num_children = count ;
...
} /* end Tk_WindowChildren() */

/*
 *--------------------------------------------------------------
 *
 * Tk_UnmapParasite, etc. --
 *
 *--------------------------------------------------------------
 */

void Tk_UnmapParasite(Tk_Window tkwin) /* Token for window to unmap. */
{
    register TkWindow *winPtr = (TkWindow *) tkwin;

    winPtr->flags &= ~TK_MAPPED;
} /* end Tk_UnmapParasite() */


void Tk_WindowCreationHandler( void (*proc)(Display *display, Window win) )
{
    global_window_creation_handlerG = proc ;
} /* end Tk_WindowCreationHandler() */

void Tk_WindowDeletionHandler( void (*proc)(Display *display, Window win) )
{
    global_window_deletion_handlerG = proc ;
} /* end Tk_WindowDeletionHandler() */

/*
 * Local Variables:
 * mode: c
