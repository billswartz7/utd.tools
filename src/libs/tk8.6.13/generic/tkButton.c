...
/* -----------------------------------------------------------------
 * Added to make the memory label work.
----------------------------------------------------------------- */
void Tk_DisplayButton(ClientData clientData)
{
    TkpDisplayButton(clientData) ;
} /* end Tk_DisplayButton() */
...
void Tk_DisplayButtonText(ClientData clientData,char *text)
{
    ...
    /* make sure we can copy text to the label without destroying data */
    valuePtr = Tcl_NewStringObj(text,-1) ;
    Tcl_DecrRefCount(butPtr->textPtr);
    butPtr->textPtr = valuePtr;
    Tcl_IncrRefCount(butPtr->textPtr);
    TkpComputeButtonGeometry(butPtr);
