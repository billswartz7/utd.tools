...
#define DEF_CANVAS_TIMBERWOLF	"0"
...
static const Tk_ConfigSpec configSpecs[] = {
	...
	TK_CONFIG_NULL_OK, NULL},
    {TK_CONFIG_BOOLEAN, "-timberwolf", "timberwolf", "TimberWolf",
 	DEF_CANVAS_TIMBERWOLF, Tk_Offset(TkCanvas, tw), 0},
...
static TkCanvas *tw_canvaS = NULL ;
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    canvasPtr->display_func = DisplayCanvas ;
    canvasPtr->scrollbar_func = CanvasUpdateScrollbars ;
#ifndef USE_OLD_TAG_SEARCH
...
void *Tk_CanvasDispFunc( void (*func)(ClientData) )
{
  if( tw_canvaS ){
    if( func ){
      tw_canvaS->display_func = func ;
    } else {
      tw_canvaS->display_func = DisplayCanvas ;
    }
  }
  return( (void *) tw_canvaS ) ;
} /* end Tk_CanvasDispFunc() */

void *Tk_CanvasScrollbarFunc( void (*scrollbar_func)(struct TkCanvas *canvas))
{
  if( tw_canvaS ){
    if( scrollbar_func ){
      tw_canvaS->scrollbar_func = scrollbar_func ;
    } else {
      tw_canvaS->scrollbar_func = CanvasUpdateScrollbars ;
    }
  }
  return( (void *) tw_canvaS ) ;
} /* end Tk_CanvasScrollbarFunc() */

void Tk_CanvasDispUpdate( void )
{
  if( tw_canvaS ){
    tw_canvaS->flags &= ~REDRAW_PENDING;
    if (tw_canvaS->flags & UPDATE_SCROLLBARS) {
      (*tw_canvaS->scrollbar_func)(tw_canvaS) ;
    }
  }
} /* end Tk_CanvasDispUpdate() */

int Tk_CanvasWidgetCmd(clientData, interp, objc, objv)
    ...
    Tcl_Obj *CONST objv[];		/* Argument objects. */
{
    return( CanvasWidgetCmd(clientData, interp, objc, objv) ) ;
} /* end Tk_CanvasWidgetCmd() */
    ...
    ClientData clientData)	/* Information about widget. */
{
  ...
  borders:
	...
	(*canvasPtr->scrollbar_func)(canvasPtr) ;
    }
}

/*
    ...
    XEvent *eventPtr)		/* Information about event. */
{
	...
	if (canvasPtr->flags & REDRAW_PENDING) {
 	    Tcl_CancelIdleCall(canvasPtr->display_func, canvasPtr);
	}
				 ...
				 * Pixels on edge are not redrawn. */
{
    ...
    if (!(canvasPtr->flags & REDRAW_PENDING)) {
 	Tcl_DoWhenIdle(canvasPtr->display_func, (ClientData) canvasPtr);
	canvasPtr->flags |= REDRAW_PENDING;
    }
}

    ...
    )
{
    ...
    if (Tcl_GetIndexFromObj(interp, objv[first], (char *) optionStrings,
