 ...
 * WM_MAX_SIZE_CMD -		hon-zero means that we're attempting to set
 *				the maximum size of a toplevel.
 */
...
#define WM_MAX_SIZE_CMD2		0x8000
...
static void		CreateWrapper(WmInfo *wmPtr,BOOL parasite);
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    wmPtr->flags |= WM_UPDATE_SIZE_HINTS;
    /*
    wmPtr->flags |= WM_UPDATE_SIZE_HINTS | WM_MAX_SIZE_CMD ;
    */
    WmUpdateGeom(wmPtr, winPtr);
    return TCL_OK;
}
    ...
    ClientData clientData)	/* Pointer to the window's record. */
{
	...
	/*
 	if( wmPtr->flags & WM_MAX_SIZE_CMD ){
 	  if( width > wmPtr->maxWidth ){
    ...
    Tcl_Obj *typePtr)
{
    ...
    wmPtr = winPtr->wmInfoPtr;
    if (wmPtr->wrapperPtr == NULL) {
	CreateWrapper(wmPtr,False);  /* FIXME */
    }
    ...
    TkWindow *winPtr)
{
    ...
    if (winPtr->wmInfoPtr->wrapperPtr == NULL) {
	CreateWrapper(winPtr->wmInfoPtr,False); /* FIXME */
    }
				 ...
				 * not be a top-level window. */
{
	...
	CreateWrapper(topPtr->wmInfoPtr,False);
    }
...
TkSetTransientFor(Tk_Window tkwin, Tk_Window parent)
{
	...
	CreateWrapper(((TkWindow *)parent)->wmInfoPtr,False); /* FIXME */
    }
...
CreateWrapper(
    WmInfo *wmPtr,		/* Window manager information for the
				 * window. */
    BOOL parasite)		/* window is a parasite??? */ 
{
    ...
    if( parasite ){
      ...
      if( XQueryTree(wrapperPtr->display,winPtr->window, &rootW, 
                     &(wrapperPtr->window), &childrenW, &num_children) == 0){
    ...
    TkWindow *winPtr)
{
    ...
    /* -----------------------------------------------------------------
     * Now safe to get rid of WM_MAX_SIZE_CMD flag.
    ----------------------------------------------------------------- */
    /* wmPtr->flags &= ~WM_MAX_SIZE_CMD; */
...
/* winPtr = Top-level window that's about to be mapped. */
void TkWmParasite(TkWindow *winPtr)
{
    ...
    TkDisplay *dispPtr ;		/* display info */

    if (wmPtr->wrapperPtr == NULL) {
      CreateWrapper(wmPtr,True);
    }
    wrapperPtr = wmPtr->wrapperPtr ;
    XSelectInput(wrapperPtr->display,wrapperPtr->window, 
	ALL_EVENTS_MASK^StructureNotifyMask) ;

    /* -----------------------------------------------------------------
     * Copy attributes of wrapper ptr over.
    ----------------------------------------------------------------- */
    XGetWindowAttributes( winPtr->display, winPtr->window, &wattr ) ;
    wrapperPtr->changes.x = wattr.x ;
    wrapperPtr->changes.y = wattr.y ;
    wrapperPtr->changes.width = wattr.width ;
    wrapperPtr->changes.height = wattr.height ;
    wrapperPtr->changes.border_width = wattr.border_width ;
    wrapperPtr->visual = wattr.visual ;
    wrapperPtr->depth = wattr.depth ;
    wrapperPtr->atts.colormap = wattr.colormap ;
    XGetWindowAttributes( wrapperPtr->display, wrapperPtr->window, &wattr ) ;
    dispPtr = wmPtr->winPtr->dispPtr ;
	...
	printf("ReparentEvent: %s reparented to 0x%x, vRoot = 0x%x\n",
		wmPtr->winPtr->pathName,
		(unsigned int) wrapperPtr->window, (unsigned int) vRoot);
    }
    ...
    handler = Tk_CreateErrorHandler(wrapperPtr->display, -1, -1, -1,
	    (Tk_ErrorProc *) NULL, (ClientData) NULL);
    wmPtr->reparent = wrapperPtr->window ;
    while (1) {
    ...
    /* -----------------------------------------------------------------
     * We also need to set the correct width and height of the window. 
     * The complicated thing below justs sets wmPtr->width and height.
    ----------------------------------------------------------------- */
    handler = Tk_CreateErrorHandler(winPtr->display, -1, -1, -1,
	    (Tk_ErrorProc *) NULL, (ClientData) NULL);
    status = XGetGeometry(winPtr->display, winPtr->window,
	    &dummy2, &x, &y, (unsigned int *) &(wmPtr->width),
	    (unsigned int *) &(wmPtr->height), (unsigned int *) &bd, &dummy);
    Tk_DeleteErrorHandler(handler);

/*
    TkWmParasiteConfigNotify((Tk_Window) winPtr) ;
    */

}/* end TkWmParasite() */

void TkWmParasiteConfigNotify(Tk_Window tkwin)
{
    WmInfo *wmPtr ;			/* window manager information */
    TkWindow *winPtr ;			/* window of interest */
    TkWindow *wrapperPtr;		/* window's wrapper window */
    XWindowAttributes wattr;		/* get window attributes of parasite */
    ...
    wrapperPtr->changes.width = wattr.width ;
    wrapperPtr->changes.height = wattr.height ;
    TkDoConfigureNotify(wrapperPtr) ;
}

int Tk_MappedWindow( Tk_Window tkwin )
{
    TkWindow *winPtr ;			/* window of interest */
    XWindowAttributes wattr;		/* get window attributes of parasite */
...
void debug_tk_id( Tk_Window tkwin )
{
    TkWindow *tk_win_p ;		/* full ptr info */
    Window win ;			/* X window id */

    tk_win_p = (TkWindow *) tkwin;
    win = Tk_WindowId( tk_win_p ) ;
    fprintf( stderr, "w:%ld id:%ld\n", (long) tkwin, (long) win ) ;
} /* end debug_tk_id() */
