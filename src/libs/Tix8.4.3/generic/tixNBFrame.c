 ...
 *
 * $Id: tixNBFrame.c,v 1.2 2019/11/14 06:13:59 bills Exp $
 */
...
typedef struct NoteBookFrameStruct {
    ...
    XColor *activeTextColorPtr;	/* Color for drawing active tab. */
    XColor *disabledFg;		/* Foreground color when disabled.  NULL
				 * means use normalFg with a 50% stipple
				 * instead. */
    GC activeGC;		/* GC for drawing text on active tab. */
    GC activeTextGC;		/* GC for drawing active tab in normal mode. */
...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_COLOR, "-activetabcolor", "activetabcolor", "ActiveTabColor",
       DEF_NOTEBOOKFRAME_ACTIVE_TAB_COLOR, Tk_Offset(WidgetRecord, 
       activeTextColorPtr), TK_CONFIG_COLOR_ONLY},
					 ...
					 * Tk_ConfigureWidget. */
{
    ...
    /*
     * Get the active text GC
     */
    gcValues.foreground = wPtr->activeTextColorPtr->pixel;
    gcValues.background = Tk_3DBorderColor(wPtr->bgBorder)->pixel;
    gcValues.font = TixFontId(wPtr->font);
    gcValues.graphics_exposures = False;
    newGC = Tk_GetGC(wPtr->tkwin,
	 GCBackground|GCForeground|GCFont|GCGraphicsExposures,
	 &gcValues);
    if (wPtr->activeTextGC != None) {
	Tk_FreeGC(wPtr->display, wPtr->activeTextGC);
    }
    wPtr->activeTextGC = newGC;
                                 ...
                                 * in */
{
	...
	if( isActive && wPtr->activeTextGC ){
	  gc = wPtr->activeTextGC;
	} else {
	  gc = wPtr->activeGC;
	}
        if (isActive) {
