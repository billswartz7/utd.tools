/*	$Id: tixInit.c,v 1.1 2018/06/30 20:04:38 bill Exp bill $ */
...
static int globalInitializedS = 0;
	 ...
	 Tcl_Interp * interp;
{
    ...
    if ((Tcl_PkgRequire(interp, "Tcl", TCL_PATCH_LEVEL, 1) == NULL)
	    || (Tcl_PkgRequire(interp, "Tk", TK_PATCH_LEVEL, 1) == NULL)) {
	return TCL_ERROR;
    }
#endif /* USE_TCL_STUBS */
    ...
    if (!globalInitializedS) {
	globalInitializedS = 1;
 ...
 * Tix_Exit --
 *
 * 	This function calls all of the Tix cleanup functions.
 *
 *----------------------------------------------------------------------
 */

int Tix_ExitCleanup(Tcl_Interp * interp)
{
    Tix_TclCmd * cmdPtr;

    for (cmdPtr = commands; cmdPtr->name != NULL; cmdPtr++) {
      Tcl_DeleteCommand(interp, cmdPtr->name ) ;
    }
    /* Delete set variables */
    Tcl_UnsetVar(interp, "tix_priv", TCL_GLOBAL_ONLY) ;
    Tcl_UnsetVar(interp, "tix_version", TCL_GLOBAL_ONLY) ;
    Tcl_UnsetVar(interp, "tix_patchLevel", TCL_GLOBAL_ONLY) ;
    Tcl_UnsetVar(interp, "tix_release", TCL_GLOBAL_ONLY) ;
    Tcl_UnsetVar(interp, "tixPriv", TCL_GLOBAL_ONLY) ;
    Tcl_UnsetVar(interp, "tix", TCL_GLOBAL_ONLY) ;
    globalInitializedS = 0;
}
