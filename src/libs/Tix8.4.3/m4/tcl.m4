...
#
# RCS: @(#) $Id: tcl.m4,v 1.1 2018/10/08 15:36:09 bill Exp bill $
...
AC_DEFUN([TEA_MAKE_LIB], [
	...
	if test "${SHARED_BUILD}" = "1" ; then
	    ...
	    eval eval "PKG_LIB_FILE=lib${SC_BUILD_ENV_SUFFIX}${PACKAGE_NAME}${SHARED_LIB_SUFFIX}"
	    RANLIB=:
	else
	    eval eval "PKG_LIB_FILE=lib${SC_BUILD_ENV_SUFFIX}${PACKAGE_NAME}${UNSHARED_LIB_SUFFIX}"
	fi
	# Some packages build their own stubs libraries
	eval eval "PKG_STUB_LIB_FILE=lib${SC_BUILD_ENV_SUFFIX}${PACKAGE_NAME}stub${UNSHARED_LIB_SUFFIX}"
    fi
