...
JS_DefineConstDoubles(JSContext *cx, JSObject *obj, JSConstDoubleSpec *cds)
{
	...
	if(strcmp((char *)cds->name, "NaN") == 0) {
	  continue ;
	}
#if defined(OSF1) || defined(__alpha__)
	else if(strcmp((char *)cds->name, "POSITIVE_INFINITY") == 0) {
	  continue ;
	} else if(strcmp((char *)cds->name, "NEGATIVE_INFINITY") == 0) {
	  continue ;
	} else if(strcmp((char *)cds->name, "MIN_VALUE") == 0) {
	  continue ;
	}
#endif /* OSF1 */
