 ...
 *
 * $Id: udp_tcl.c,v 1.1 2024/08/05 13:29:26 billswartz7 Exp billswartz7 $
 ******************************************************************************/
...
int udpInfo(ClientData , Tcl_Interp *, int , Tcl_Obj * CONST []) ;
...
Udp_Init(Tcl_Interp *interp)
{
    ...
    Tcl_CreateCommand(interp, "udp_peek", udpPeek , 
                      (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateObjCommand(interp, "udp_info", udpInfo,
                      (ClientData) NULL, (Tcl_CmdDeleteProc *) NULL);
...
int udpInfo(ClientData dp,Tcl_Interp *i_p,int objc, Tcl_Obj * CONST84 objv[])
{

    extern int errno ;			/* error number */
    char hostbuf[256] ;			/* space for host name */
    struct hostent *host_entry ;	/* get info about host */
    struct in_addr **addr_list ;	/* host address lists */
    Tcl_Obj *strObj ;			/* temp string object */
    Tcl_Obj *resultPtr ;		/* set the Tcl result object */
    int i ;				/* interface counter */
    int hostname_status ;		/* return code */ 

    /* get hostname */
    hostname_status = gethostname(hostbuf, sizeof(hostbuf) ) ;
    if ( hostname_status == -1 ) {
      resultPtr = Tcl_GetObjResult(i_p) ;
      strObj = Tcl_NewStringObj("gethostname error", -1 ) ;
      Tcl_ListObjAppendElement(NULL,resultPtr, strObj) ;
      strObj = Tcl_NewStringObj( strerror(errno), -1 ) ;
      Tcl_ListObjAppendElement(NULL,resultPtr, strObj) ;
      return(TCL_ERROR) ;
    }
    ...
    if (!(host_entry)) {
      resultPtr = Tcl_GetObjResult(i_p) ;
      strObj = Tcl_NewStringObj("gethostbyname error", -1 ) ;
      Tcl_ListObjAppendElement(NULL,resultPtr, strObj) ;
      strObj = Tcl_NewStringObj( strerror(errno), -1 ) ;
      Tcl_ListObjAppendElement(NULL,resultPtr, strObj) ;
      return(TCL_ERROR) ;
    ...
    for ( i = 0 ; addr_list[i] ; i++ ) {
      strObj = Tcl_NewStringObj( inet_ntoa(*addr_list[i]), -1 ) ;
      Tcl_ListObjAppendElement(NULL, resultPtr, strObj ) ;
    }
    return( TCL_OK ) ;
} /* end udpInfo() */
