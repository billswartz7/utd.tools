    ...
    const Tcl_Time *timePtr)		/* Timeout value, may be NULL. */
{
	...
	notifier.currentTimeout = XtAppAddTimeOut(notifier.appContext,
		(unsigned long) timeout,(XtTimerCallbackProc)TimerProc, NULL);
    } else {
    ...
    ClientData clientData)	/* Arbitrary data to pass to proc. */
{
	...
	if (!(filePtr->mask & TCL_READABLE)) {
	    filePtr->read = XtAppAddInput(notifier.appContext, fd,
		    INT2PTR(XtInputReadMask), 
		    (XtInputCallbackProc)FileProc, filePtr);
	}
    } else {
	if (filePtr->mask & TCL_READABLE) {
