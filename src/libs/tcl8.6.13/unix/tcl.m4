...
# SC_BUGGY_READDIR_R
#
#	Under some Linux versions, readdir_r exists but doesn't work
#       properly.  Don't define HAVE_READDIR_R in this case.
#
# Arguments:
#	none
#	
...
AC_DEFUN(SC_BUGGY_READDIR_R, [
    AC_CHECK_FUNC(readdir_r, tcl_readdir_r=1, tcl_readdir_r=0)
    if test "$tcl_readdir_r" = 1; then
	AC_MSG_CHECKING([for working readdir_r])
	AC_CACHE_VAL(tcl_cv_readdir_r_buggy,[
	    AC_TRY_RUN([
		  #include <stdio.h>
		  #define _REENTRANT
		  ...
		  main()
		  {
		    ...
		    char path[BUFSIZ+1] ;
		    DIR *dir_p ;
		    struct dirent *dirbuf_p ;
		    struct dirent *dirEntPtr ;

		    source = (char *) getcwd(path,BUFSIZ) ;
		    dir_p = opendir(source) ;
		    dirbuf_p = (struct dirent *) malloc( sizeof(struct dirent) + 1024) ;
		    val =  readdir_r(dir_p,dirbuf_p,&dirEntPtr) ;
		    if( val == 0 && dirEntPtr ){
		      exit(0) ;
		    }
		    exit(1) ;
		}], tcl_cv_readdir_r_buggy=1, tcl_cv_readdir_r_buggy=0, tcl_cv_readdir_r_buggy=0)])
	if test "$tcl_cv_readdir_r_buggy" = 1; then
	    AC_MSG_RESULT(ok)
	    AC_DEFINE(HAVE_READDIR_R)
	else
	    AC_MSG_RESULT(buggy use readdir instead)
	fi
    fi
])
