...
extern int		Tcl_DictionaryCompare (const char *left,const char *right,unsigned long len) ;
    ...
    Tcl_Obj *const objv[])	/* Argument values. */
{
	...
	while (lower + 1 != upper && sortInfo.resultCode == TCL_OK) {
	    ...
	    case DICTIONARY:
		bytes = TclGetString(itemPtr);
 		match = Tcl_DictionaryCompare(patternBytes, bytes,0);
		break;
	    case INTEGER:
	...
	for (i = offset; i < listc; i++) {
		...
		case DICTIONARY:
		    bytes = TclGetString(itemPtr);
 		    match = (Tcl_DictionaryCompare(bytes, patternBytes,0) == 0);
		    break;
				 ...
				 * "lsort" command. */
{
	...
	order = Tcl_DictionaryCompare(elemPtr1->collationKey.strValuePtr,
		elemPtr2->collationKey.strValuePtr,0) ;
...
int
Tcl_DictionaryCompare(
    const char *left, const char *right,unsigned long len)	/* The strings to compare. */
{
