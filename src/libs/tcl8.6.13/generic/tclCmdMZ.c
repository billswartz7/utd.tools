...
extern int Tcl_DictionaryCompare(const char *left, const char *right,unsigned long length) ;
 ...
 * Tcl_String_Distance
 *
 *	This procedure computes a double which roughly shows the distance 
 *	between two strings.   If the two strings are equal 0 is returned.
 *
 * Results:
 *	Distance between two string: double
 *
 *----------------------------------------------------------------------
 */
static double Tcl_String_Distance( Tcl_Obj *string_obj1, Tcl_Obj *string_obj2,
                            int nocase)
{
    int length ;		/* common length */
    int length1 ;		/* length of first string */
    int length2 ;		/* length of second string */
    double diff ;		/* difference between current two characters */
    double dist ;		/* distance between two string */
    double mult ;		/* only 128 ascii characters */
    const char *string1 ;	/* first string */
    const char *string2 ;	/* second string */

    dist = 0.0 ;
    string1 = Tcl_GetStringFromObj( string_obj1, &length1);
    string2 = Tcl_GetStringFromObj( string_obj2, &length2);
    if( length1 >= length2 ){
      length = length1 ;
    } else {
      length = length2 ;
    ...
    for( ; *string1 && *string2 ; string1++, string2++ ){
      if( nocase ){  /* -nocase option */
	diff = (tolower(*string1) - tolower(*string2)) * mult ;
      } else {
	diff = (*string1 - *string2) * mult ;
      }
      if( diff < 0 ){
	dist = dist - diff ;
      } else {
	dist += diff ;
      ...
      mult /= 128.0 ;  /* 128 Ascii chars */
    }
    if( length1 != length2 ){
      dist += 1 ;
    }
    return( dist ) ;
} /* end Tcl_String_Distance() */
 ...
 * StringDistCmd --
 *
 *	This procedure is invoked to process the "string distance" Tcl command.
 *	See the user documentation for details on what it does. Note that this
 *	command only functions correctly on properly formed Tcl UTF strings.
 *
 * Results:
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    if (objc != 3 && objc != 4) {
	Tcl_WrongNumArgs(interp, 1, objv, "string1 string2 [-nocase]");
	return TCL_ERROR;
    }
    if( objc == 4 ){
      dist = Tcl_String_Distance( objv[1], objv[2], 1 ) ;
    } else {
      dist = Tcl_String_Distance( objv[1], objv[2], 0 ) ;
    }
    ...
    Tcl_Interp *interp)		/* Current interpreter. */
{
	...
	{"distance",	StringDistCmd,	NULL, NULL, NULL, 0},
