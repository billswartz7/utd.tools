 ...
 *----------------------------------------------------------------------
 *
 * Tcl_ChannelHandlerExists --
 *
 *	See if a channel handler exists for a given channel,  Use options
 *	to control search.
 *
 * Results:
 *	True if matching handler is found.  False otherwise.
 *
 * Side effects:
 *	None.
 *
...
int
Tcl_ChannelHandlerExists(chan, proc, clientData, options)
    Tcl_Channel chan;		/* The channel to create the handler for. */
    Tcl_ChannelProc *proc;	/* Procedure to call for each
				 * selected event. */
    ClientData clientData;	/* Arbitrary data to pass to proc. */
    int options;		/* If true, need to match clientData as well. */
{
    ChannelHandler *chPtr;
    Channel *chanPtr = (Channel *) chan;
    ChannelState *statePtr = chanPtr->state;	/* state info for channel */
    ...
    for (chPtr = statePtr->chPtr; chPtr ; chPtr = chPtr->nextPtr) {
        if ((chPtr->chanPtr == chanPtr) && (chPtr->proc == proc) ){
	    if( options && (chPtr->clientData != clientData)) {
	      continue ; /* not really the one */
	    }
	    return 1 ;
        }
    }
