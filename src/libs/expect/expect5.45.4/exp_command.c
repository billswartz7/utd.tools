    ...
    Tcl_Obj *CONST objv[])		/* Argument objects. */
{
    ...
    int nowait = FALSE;
    int target_pid ;		/* wait for a specific pid */
    int save_pid ;		/* save process id */
...
#define NO_CHILD (-2)
	...
	switch ((enum options) index) {
		...
		i++;
		if (i >= objc) goto usage;
		if (TCL_OK != Tcl_GetIntFromObj (interp, objv[i], &target_pid)) {
		    return TCL_ERROR;
		}
		break;
	}
	...
	if (!esPtr->sys_waited || target_pid) {
	    if (nowait) {
