...
#
# RCS: @(#) $Id: tcl.m4,v 1.1 2021/09/14 19:05:59 billswartz7 Exp billswartz7 $
...
AC_DEFUN([TEA_PROG_TCLSH], [
    ...
    if test -f "${TCL_BIN_DIR}/Makefile" ; then
        ...
        if test "${TEA_PLATFORM}" = "windows"; then
            TCLSH_PROG="${TCL_BIN_DIR}/cometTclsh${TCL_MAJOR_VERSION}${TCL_MINOR_VERSION}${TCL_DBGX}${EXEEXT}"
        else
            TCLSH_PROG="${TCL_BIN_DIR}/cometTclsh"
        fi
    else
        # tclConfig.sh is in install location
        if test "${TEA_PLATFORM}" = "windows"; then
            TCLSH_PROG="cometTclsh${TCL_MAJOR_VERSION}${TCL_MINOR_VERSION}${TCL_DBGX}${EXEEXT}"
        else
            TCLSH_PROG="cometTclsh${TCL_MAJOR_VERSION}.${TCL_MINOR_VERSION}${TCL_DBGX}"
        fi
...
dnl AC_CHECK_TOOL(AR, ar)
	...
	FreeBSD-*)
	    ...
	    UNSHARED_LIB_SUFFIX='${TCL_TRIM_DOTS}.a'
	    # This was the original but it doesn't work on my system
	    # and no link is created to it in the Makefile.in file.
	    # So just remove the .1 - WPS.
...
dnl # preprocessing tests use only CPPFLAGS.
	...
	# TEA specific: use PACKAGE_VERSION instead of VERSION
	SHARED_LIB_SUFFIX='${SHLIB_SUFFIX}'])
