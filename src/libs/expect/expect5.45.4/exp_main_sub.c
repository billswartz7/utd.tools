...
#endif
char exp_version[] = "expect.4.54.3" ;
...
Tcl_Interp *interp;
{
     ...
     */
    /* Set to UTDTOOLS variables */
    utdtools_p = utDgetenv("UTDTOOLS") ;
    version_p = utDgetenv("UTDTOOLSVERSION") ;
    if( utdtools_p && version_p ){
      utDdstring_init(&msg_buf) ;
      msg_p = utDdstring_printf( &msg_buf, "%s/%s/tcl/expect", utdtools_p, version_p ) ;
      if (Tcl_GetVar(interp, "expect_library", TCL_GLOBAL_ONLY) == NULL) {
	  Tcl_SetVar(interp,"expect_library",msg_p,0);/* deprecated */
      }
      if (Tcl_GetVar(interp, "exp_library", TCL_GLOBAL_ONLY) == NULL) {
	  Tcl_SetVar(interp,"exp_library",msg_p,0);
      }
      if (Tcl_GetVar(interp, "exp_exec_library", TCL_GLOBAL_ONLY) == NULL) {
	  Tcl_SetVar(interp,"exp_exec_library",msg_p,0);
      }
