...
static void scrollbar_update(TkCanvas *canvasPtr) ;
...
static void setup_X( Tcl_Interp *interp, GraphicsContextPtr gc_p ) {
   ...
   * Highest priority is the geometry gcS->override.
...
static void cleanup_x( GraphicsContextPtr gc_p, void (*refresh_func)(UTDTILERECTPTR,void *) ) {
  ...
  Tk_CreateForcedEventHandler( gc_p->drawWindow, 
    StructureNotifyMask | SubstructureNotifyMask, check_reconfig, NULL) ;
  ...
  if( refresh_func ){
    ...
    Tk_CanvasScrollbarFunc( (Tk_function) scrollbar_update ) ;
  }
  ...
  ----------------------------------------------------------------- */
  //if( user_initialization(gcS->interp) ){
//	update_events(TK_DONT_WAIT) ;
//     }
