...
char *Blt_Itoa(INT value,char *buffer)
{
 ...
 */
static void
InitSink(BackgroundInfo *bgPtr,Sink *sinkPtr,char *name,Tcl_Encoding encoding)
{
    sinkPtr->name = name;
    sinkPtr->echo = FALSE;
    sinkPtr->fd = -1;
	...
	objArr = (Tcl_Obj **) ckalloc( (count + 1) * sizeof(Tcl_Obj *) ) ;
	for (i = 0; i < count; i++) {
...
ExtendSinkBuffer(Sink *sinkPtr)
{
    ...
    arrayPtr = ckalloc( sinkPtr->size * sizeof(unsigned char) ) ;
    if (arrayPtr == NULL) {
	return -1;
    }
...
CookSink(Tcl_Interp *interp,Sink *sinkPtr)
{
	...
	cooked = ckalloc( cookedSize * sizeof(unsigned char) ) ;
...
static void
NotifyOnUpdate(Tcl_Interp *interp,Sink *sinkPtr,unsigned char *data,int nBytes)
{
    char save;
...
static int
CollectData(BackgroundInfo *bgPtr,Sink *sinkPtr)
{
 ...
 */
static int
CreateSinkHandler(BackgroundInfo *bgPtr,Sink *sinkPtr,Tcl_FileProc *proc)
{
#ifndef WIN32
    int flags;
    char buffer[LRECL] ;
	...
	Tcl_AppendResult(bgPtr->interp, "can't set file descriptor ",
	    Blt_Itoa(sinkPtr->fd,buffer), " to non-blocking:",
	    Tcl_PosixError(bgPtr->interp), (char *)NULL);
	return TCL_ERROR;
    }
#endif /* WIN32 */
...
static void
DisableTriggers(BackgroundInfo *bgPtr)	/* Background info record. */
{
    int k;
 ...
 */
static void
FreeBackgroundInfo(BackgroundInfo *bgPtr)
{
	...
	ckfree((char *)bgPtr->procArr);
    }
    if (bgPtr->timerTokens != NULL) {
	ckfree((char *)bgPtr->timerTokens);
    }
    ckfree((char *)bgPtr);
}
...
/* ARGSUSED */
static void
DestroyBackgroundInfo(BackgroundInfo *bgPtr)	/* Background info record. */
{
...
#else
    Tcl_DStringAppendElement(&dString, Blt_Itoa(lastPid,buffer));
#endif
    Tcl_DStringAppendElement(&dString, Blt_Itoa(code,buffer));
    switch(pcode) {
    case PROCESS_EXITED:
    ...
    int interval;
{
    if(!(bgPtr->timerTokens)){
      bgPtr->timerTokens = 
	(Tcl_TimerToken *) ckalloc(EXPECTED_NUM_TIMERS * sizeof(Tcl_TimerToken)) ;
      memset( bgPtr->timerTokens, 0L, 
	      (EXPECTED_NUM_TIMERS * sizeof(Tcl_TimerToken) ) ) ;
    }
    if( ++bgPtr->nTimers >= EXPECTED_NUM_TIMERS ){
      bgPtr->timerTokens = (Tcl_TimerToken *) 
		  ckrealloc( (VOID *) bgPtr->timerTokens, 
    ...
    char **argv;		/* Argument strings. */
{
    ...
    bgPtr = (BackgroundInfo *) ckalloc( sizeof(BackgroundInfo) ) ;
    memset( bgPtr, 0L, sizeof(BackgroundInfo) ) ;
    /* assert(bgPtr); */
...
static Tcl_Command Blt_InitCmd(Tcl_Interp *interp,char *nsName,
			       Blt_CmdSpec *specPtr)
{
    char *cmdPath;
    Tcl_DString dString;
