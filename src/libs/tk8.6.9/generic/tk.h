...
typedef enum {
    ...
    TK_CONFIG_END, TK_CONFIG_PARASITE
} Tk_ConfigTypes;
...
/*
 * Set/get the busy signal.
 */
#define TK_BUSY_GET   (-1)

/*
 *----------------------------------------------------------------------
 *
 ...
 * TK_PARASITE:			1 means that this window is a parasite and
 *				was not created by Tk.  This window should
 *				not be destroyed.
 */
 ...
 *----------------------------------------------------------------------
 */
typedef void (*Tk_function)(void *clientData) ;
...
typedef int (Tk_DefaultErrorProc) (Display *display,XErrorEvent *errEventPtr);
