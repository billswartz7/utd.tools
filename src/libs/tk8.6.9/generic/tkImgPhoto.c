...
struct SubcommandOptions {
    ...
    int compositingRule;	/* Value specified for -compositingrule option. */
    double scale;		/* scaling option */
};
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
	...
	"read", "redither", "reduce", "scale", "transparency", "write", NULL
    };
    enum PhotoOptions {
	PHOTO_BLANK, PHOTO_CGET, PHOTO_CONFIGURE, PHOTO_COPY, PHOTO_DATA,
	PHOTO_GET, PHOTO_PUT, PHOTO_READ, PHOTO_REDITHER, PHOTO_REDUCE,
	PHOTO_SCALE, PHOTO_TRANS, PHOTO_WRITE
    };
	...
	/*
	options.scale = 0.75 ;
	if (ParseSubcommandOptions(&options, interp, 0,
		&index, objc, objv) != TCL_OK) {
	    return TCL_ERROR;
	}
	if ((options.name != NULL) || (index < objc)) {
	...
	*/
	 ...
	 */
	/* old_data = data ; */
	pixel_data = Tcl_GetString(objv[2]) ;
	data = TkImgQuantize( pixel_data ) ;
	...
	options.fromY = 0;
	options.scale = 0.75 ;
	if (ParseSubcommandOptions(&options, interp, OPT_SCALE,
		&index, objc, objv) != TCL_OK) {
	    return TCL_ERROR;
	}
	/*
	...
	*/
	...
	if( options.scale <= 0.0 ){
	  Tcl_AppendResult(interp, "scale less than 0.0 ", (char *) NULL);
	  return TCL_ERROR;
	}
	if( options.scale > 1.0 ){
	  Tcl_AppendResult(interp, "scale greater than 1.0. ", (char *) NULL);
	  return TCL_ERROR;
	}
	/* old_data = data ; */
	new_width = (int) floor( (double)masterPtr->width * options.scale ) ;
	new_height = (int) floor( (double)masterPtr->height * options.scale ) ;
	if( new_width == 0 ){
	  new_width = 1 ;
	}
	if( new_width == 0 ){
	...
	pixel_data = Tcl_GetString(objv[2]) ;
	data = TkImgShrink( pixel_data, new_width, new_height ) ;
	Tcl_SetObjResult(interp, Tcl_NewStringObj(data,-1) ) ;
    ...
    Tcl_Obj *const objv[])	/* Arguments to be parsed. */
{
	...
	if (bit == OPT_BACKGROUND) {
 	  ...
 	  if (index + 1 < objc) {
 	      *optIndexPtr = ++index;
 	      if( Tcl_GetDoubleFromObj(interp,objv[index],&(optPtr->scale) ) !=
 		  TCL_OK ){
 		return TCL_ERROR;
 	      }
 	  } else {
