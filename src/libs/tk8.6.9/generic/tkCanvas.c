...
static const Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_BOOLEAN, "-simple", "simple", "Simple",
 	DEF_CANVAS_SIMPLE, Tk_Offset(TkCanvas, simple), 0},
...
static TkCanvas *simple_canvaS = NULL ;
...
void *Tk_CanvasDispFunc( Tk_function func )
{
  if( simple_canvaS ){
    if( func ){
      simple_canvaS->display_func = func ;
    } else {
      simple_canvaS->display_func = DisplayCanvas ;
    }
  }
  return( (void *) simple_canvaS ) ;
} /* end Tk_CanvasDispFunc() */

void *Tk_CanvasScrollbarFunc( Tk_function scrollbar_func )
{
  if( simple_canvaS ){
    if( scrollbar_func ){
      simple_canvaS->scrollbar_func = (Tk_scrollbar_func) scrollbar_func ;
    } else {
      simple_canvaS->scrollbar_func = CanvasUpdateScrollbars ;
    }
  }
  return( (void *) simple_canvaS ) ;
...
{
  if( simple_canvaS ){
    simple_canvaS->flags &= ~REDRAW_PENDING;
    if (simple_canvaS->flags & UPDATE_SCROLLBARS) {
      (*simple_canvaS->scrollbar_func)(simple_canvaS) ;
    }
  }
				 ...
				 * Pixels on edge are not redrawn. */
{
    ...
    if (!(canvasPtr->flags & REDRAW_PENDING)) {
	Tcl_DoWhenIdle( canvasPtr->display_func, canvasPtr);
	canvasPtr->flags |= REDRAW_PENDING;
    }
}

