...
# XF version: 4.0
# $Log: colors.tcl,v $
# Revision 1.1  2018/10/10 21:13:12  bill
# Initial revision
#
#
#
...
namespace eval utdtoolsgui {
  ...
  proc color_display {args} {
    ...
    wm positionfrom .colors ""
    wm sizefrom .colors ""
    wm maxsize .colors 800 800
    wm minsize .colors 10 10
    wm title .colors {Display Colors}

    # build widget .colors.frame0
    frame .colors.frame0 \
      -borderwidth {2} \
      -relief {raised}
      
    # build widget .colors.frame3
    frame .colors.frame3 \
      -borderwidth {2} \
      -height {30} \
      -relief {raised} \
    ...
    # build widget .colors.frame3.label1
    label .colors.frame3.label1 \
      -text {Colors}

    
    # build widget .colors.frame1.button3
    button .colors.frame0.button0 \
      ...
      -padx {9} \
      -pady {3} \
      -command {destroy .colors} \
      -text {ACCEPT}

    # build widget .colors.frame1.button3
    button .colors.frame0.button1 \
      -activebackground {White} \
      ...
      -padx {9} \
      -pady {3} \
      -command {destroy .colors; ::utdtoolsgui::restore} \
      -text {CANCEL}

    # pack master .colors.frame0
    pack configure .colors.frame0.button0 \
      -padx 40 \
      -side left
    pack configure .colors.frame0.button1 \
      -padx 40 \
      -side right

    # build widget .colors.frame1
    frame .colors.frame1 \
      -borderwidth {2} \
      -relief {raised}

    tixScrolledWindow .colors.frame1.scroll_window
    pack .colors.frame1.scroll_window -expand yes -fill both \
      -padx 10 -pady 10 -side left
    set a [.colors.frame1.scroll_window subwidget window]
    ...
    set greset [frame .colors.frame2.global_resetf -borderwidth {2} -relief ridge]
	...
	-command "::utdtoolsgui::global_reset $a true ;orredraw"
    button $greset.reset_off -borderwidth {1} -pady {0} -text {all colors off} \
	-command "::utdtoolsgui::global_reset $a false ; orredraw"
    ...
    pack configure .colors.frame3.label1
    
    # pack master .colors
    pack configure .colors.frame3 \
      -fill x \
      -expand false
      
    pack configure .colors.frame0 \
      -fill x
   
    pack configure .colors.frame2

    pack configure .colors.frame1 \
      -fill x \
      -expand yes
  ...
  proc utdcolors_build_stipple {w col} {
	...
	if {[catch {label $w.stipple$i -relief {groove} -bitmap $stip \
	  ...
	  otmessage warnmsg utdcolors_build_stipple "$msg for color index:$i\n"
	  label $w.stipple$i \
	     -relief {flat} \
	     -text "none" \
	     -fg [utdcolors name $i]
  ...
  proc utdcolors_build_display {w col} {
    ...
    for {set i 1} {$i <= $num_colors} {incr i 1} {
	  ...
	  -text {on} \
	  -value {1} \
	  -variable ::utdtoolsgui::colorS($i) \
	  -command "utdcolors draw $i ; orredraw"
	  ...
	  -value {0} \
	  -variable ::utdtoolsgui::colorS($i) \
	  -command "utdcolors ignore $i ; orredraw"
