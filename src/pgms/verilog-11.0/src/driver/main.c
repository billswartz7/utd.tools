...
#ifndef __MINGW32__
	/* First try UTDTOOLS environment path */
      extern char *getenv(const char *) ;
      char *utdtools = getenv("UTDTOOLS") ;
      char *utdversion = getenv("UTDTOOLSVERSION") ;
      char *utd_os = getenv("UTDTOOLSOS") ;
      int name_len ;

      if( utdtools && utdversion && utd_os ){
	name_len = strlen(utdtools) + strlen(utdversion) + strlen(utd_os) + 4 ;
	if( name_len < MAXSIZE ){
	  sprintf( ivl_root, "%s/%s/lib/%s%s",
	      utdtools, utdversion, utd_os, ivl_lib_suffix ) ;
      ...
      /* As fall back, try the location specified in the build process. */
	/*
      if (access(IVL_ROOT, F_OK) != -1) {
