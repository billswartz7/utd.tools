...
static void lex_param(void) ;
static void yparse_param(void) ;
...
static char *Ystrclone(char *str,BOOL calc_length) ;
...
static char yparse_param_seenS, lex_param_seenS ;
char *cptr, *line;
int linesize;
...
int name_pool_size;
char *name_pool;
PARAMPTR lex_paramsG;
PARAMPTR yparse_paramsG;
...
char *Ystrclone(char *str,BOOL calc_length)
{
    int len ;			/* length of string */
    char *s ;			/* current position in string */
    char *new_string ;		/* newly created string */
...
char *Ystrtok(char *str, char *dels)
/* input string */
...
{
    int a ;				/* counter */
    PARAMPTR last_param_p ;		/* newly created parameter */

    char *y_line = dup_line();
    char *y_cptr = y_line + (cptr - line);
    char *param_p ;		/* current parameter */

    if( yparse_param_seenS ){
/*      multi_yparse_param(cptr - 12) ; */
    }
    yparse_param_seenS = 1 ;
    param_p = NULL ;
    for( ; *y_cptr ; y_cptr++ ){
      if ( *y_cptr == '{' ){
	if( !(param_p) ){
    ...
    if( !(param_p)){
      syntax_error(lineno, y_line, cptr ) ;
    }
    for( y_cptr++; *y_cptr ; y_cptr++ ){
      if ( *y_cptr == '{' ){
	syntax_error(lineno, y_line, y_cptr ) ;
      } else if ( *y_cptr == '}' ){
	int argc ;
	char *parser_toks[80] ;
	param_p++ ;
	char *copy_params = Ystrclone( param_p, TRUE ) ;
	char **argv = Ystrparser2( copy_params, " {}\t\r\n", parser_toks, &argc ) ;
	if( argc <= 0 ){
	  syntax_error(lineno, y_line, y_cptr ) ;
	} else if ( argc % 2 ){
	  syntax_error(lineno, y_line, y_cptr ) ;
	...
	for( a = 0 ; a < argc ; a++ ){
	  if( yparse_paramsG ){
	    last_param_p->next_param = MALLOC(sizeof(PARAMBOX)) ;
	    last_param_p = last_param_p->next_param ;
	  } else {
	    yparse_paramsG = last_param_p = MALLOC(sizeof(PARAMBOX)) ;
	  }
	  last_param_p->next_param = NULL ;
	  last_param_p->arg_type = Ystrclone( argv[a], FALSE ) ;
	  last_param_p->arg_name = Ystrclone( argv[++a], FALSE ) ;
	}
	FREE(copy_params);
	FREE(y_line);
	get_line();
...
/*      multi_yparse_param(cptr - 12) ; */
    ...
    if( !(param_p)){
      syntax_error(lineno, l_line, cptr ) ;
    }
    for( l_cptr++; *l_cptr ; l_cptr++ ){
      if ( *l_cptr == '{' ){
	syntax_error(lineno, l_line, l_cptr ) ;
      } else if ( *l_cptr == '}' ){
	int argc ;
	char *parser_toks[80] ;
	param_p++ ;
	...
	for( a = 0 ; a < argc ; a++ ){
	  if( lex_paramsG ){
	    ...
	    lex_paramsG = last_param_p = MALLOC(sizeof(PARAMBOX)) ;
	  }
	  last_param_p->next_param = NULL ;
	  last_param_p->arg_type = Ystrclone( argv[a], FALSE ) ;
	  last_param_p->arg_name = Ystrclone( argv[++a], FALSE ) ;
