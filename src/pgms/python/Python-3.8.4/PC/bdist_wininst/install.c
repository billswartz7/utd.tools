 ...
 *
 * $Id: install.c,v 1.1 2020/07/16 14:15:32 bill Exp bill $
 */
...
static HINSTANCE LoadPythonDll(char *fname)
{
    ...
    /* make sure UTDPYTHONHOME is set, to that sys.path is initialized correctly */
    wsprintf(buffer, "UTDPYTHONHOME=%s", python_dir);
    _putenv(buffer);
    h = LoadLibrary(fname);
    if (h)
        return h;
...
static LPSTR get_sys_prefix(LPSTR exe, LPSTR dll)
{
    ...
    {
        char Buffer[256];
        wsprintf(Buffer, "UTDPYTHONHOME=%s", exe);
        *strrchr(Buffer, '\\') = '\0';
//      MessageBox(GetFocus(), Buffer, "UTDPYTHONHOME", MB_OK);
                _putenv(Buffer);
                _putenv("UTDPYTHONPATH=");
    }
