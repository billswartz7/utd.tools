...
/* ----------------------------------------------------------------
   ...
   * If the UTDPYTHONPATH env. var. exists, its entries are added next.
   ...
   * We attempt to locate the "Python Home" - if the UTDPYTHONHOME env var
   ...
   * Iff - we can not locate the Python Home, have not had a UTDPYTHONPATH
...
typedef struct {
    ...
    const wchar_t *home;               /* UTDPYTHONHOME environment variable */
...
/* Load a UTDPYTHONPATH value from the registry.
                             ...
                             const wchar_t *zip_path)
{
...
#endif
    /* We only use the default relative UTDPYTHONPATH if we haven't
    ...
    /* We need to construct a path from the following parts.
       (1) the UTDPYTHONPATH environment variable, if set;
       (2) for Win32, the zip archive file path;
       (3) for Win32, the machine_path and user_path, if set;
       (4) the UTDPYTHONPATH config macro, with the leading "."
           of each component replaced with home, if set;
       (5) the directory containing the executable (argv0_path).
       The length calculation calculates #4 first.
       Extra rules:
       - If UTDPYTHONHOME is set (in any way) item (3) is ignored.
    ...
    */
    ...
    if (calculate->home == NULL) {
        if (!skipdefault) {
            if (wcscpy_s(buf, bufsz - (buf - start_buf), UTDPYTHONPATH)) {
                return INIT_ERR_BUFFER_OVERFLOW();
            }
            buf = wcschr(buf, L'\0');
            *buf++ = DELIM;
