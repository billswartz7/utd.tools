...
pymain_run_startup(PyConfig *config, PyCompilerFlags *cf, int *exitcode)
{
...
#ifdef MS_WINDOWS
    const wchar_t *wstartup = _wgetenv(L"UTDPYTHONSTARTUP");
...
#else
    const char *startup = _Py_GetEnv(config->use_environment, "UTDPYTHONSTARTUP");
    if (startup == NULL) {
        return 0;
    }
    ...
    if (fp == NULL) {
        ...
        PyErr_Clear();
        PySys_WriteStderr("Could not open UTDPYTHONSTARTUP\n");
...
pymain_repl(PyConfig *config, PyCompilerFlags *cf, int *exitcode)
{
    ...
    if (!config->inspect && _Py_GetEnv(config->use_environment, "UTDPYTHONINSPECT")) {
        config->inspect = 1;
        Py_InspectFlag = 1;
    }
