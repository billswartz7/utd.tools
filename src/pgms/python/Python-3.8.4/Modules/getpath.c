 ...
 * installed setup.
 *
 * Step 2. See if the $UTDPYTHONHOME environment variable points to the
 * installed location of the Python libraries.  If $UTDPYTHONHOME is set, then
 * it points to prefix and exec_prefix.  $UTDPYTHONHOME can be a single
 ...
 * preprocessor variable UTDPYTHONPATH is used to construct a path.  Each
 * relative path on UTDPYTHONPATH is prefixed with prefix.  Then the directory
 * containing the shared library modules is appended.  The environment
 * variable $UTDPYTHONPATH is inserted in front of it all.  Finally, the
                             ...
                             const wchar_t *zip_path)
{
    ...
    /* Next goes merge of compile-time $UTDPYTHONPATH with
...
calculate_init(PyCalculatePath *calculate, const PyConfig *config)
{
    ...
    calculate->pythonpath = Py_DecodeLocale(UTDPYTHONPATH, &len);
    if (!calculate->pythonpath) {
        return DECODE_LOCALE_ERR("UTDPYTHONPATH define", len);
    }
...
calculate_path(PyCalculatePath *calculate, _PyPathConfig *pathconfig)
{
    ...
    {
        fprintf(stderr,
                "Consider setting $UTDPYTHONHOME to <prefix>[:<exec_prefix>]\n");
    }
...
/* Calculate the Python path configuration.
   ...
   - Macros: UTDPYTHONPATH, PREFIX, EXEC_PREFIX, VERSION (ex: "3.9").
     PREFIX and EXEC_PREFIX are generated by the configure script.
     UTDPYTHONPATH macro is the default search path.
   - pybuilddir.txt file
   - pyvenv.cfg configuration file
   - PyConfig fields ('config' function argument):

     - pathconfig_warnings
     - pythonpath_env (UTDPYTHONPATH environment variable)

   - _PyPathConfig fields ('pathconfig' function argument):

     - program_name: see config_init_program_name()
     - home: Py_SetPythonHome() or UTDPYTHONHOME environment variable
