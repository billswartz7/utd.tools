...
-E     : ignore UTDPYTHON* environment variables (such as UTDPYTHONPATH)\n\
...
UTDPYTHONPATH : '%lc'-separated list of directories prefixed to the\n\
               default module search path.  The result is sys.path.\n\
";
static const char usage_5[] =
"UTDPYTHONHOME : alternate <prefix> directory (or <prefix>%lc<exec_prefix>).\n"
...
#  define UTDPYTHONHOMEHELP "<prefix>\\python{major}{minor}"
#else
#  define UTDPYTHONHOMEHELP "<prefix>/lib/pythonX.X"
#endif
...
int Py_IgnoreEnvironmentFlag = 0; /* e.g. UTDPYTHONPATH, UTDPYTHONHOME */
...
config_usage(int error, const wchar_t* program)
{
    ...
    if (error)
        ...
        fprintf(f, usage_5, (wint_t)DELIM, UTDPYTHONHOMEHELP);
        fputs(usage_6, f);
    }
}
...
#define DUMP_CONFIG(NAME, FIELD) \
    ...
    DUMP_CONFIG("UTDPYTHONHOME", home);
    DUMP_CONFIG("UTDPYTHONPATH", pythonpath_env);
